...
uint64_t proc_pcomm_offset(uint16_t fw) {
    if ((fw & 0xFFFD) == 0x1F9) {
        return 0x44C;
    }
    uint16_t diff_671 = (uint16_t)(fw - 0x29F);
    uint16_t diff_702 = (uint16_t)(fw - 0x2BE);
    int match = 1;
    if (diff_702 <= 0x35) {

        uint64_t bitmap = 0xFFDCFFFFFFFFFFFELL;
        match = (bitmap >> diff_702) & 1;
    }
    if (diff_671 <= 1 || !match) {
        return 0x454;
    }
    return (fw < 0x384) ? 0x44C : 0x454;
}
...
struct proc *proc_find_by_name(const char *name) {
    ...
    uint64_t pcomm_off = proc_pcomm_offset(cachedFirmware);
    struct proc *p = *allproc;
    do {
        if (!memcmp((const char *)p + pcomm_off, name, strlen(name))) {
            return p;
        }
    } while ((p = p->p_forw));
    ...
    for (int i = 0; i < num_entries; i++) {
        ...
        if (!memcmp(entries[i].name, "libkernel.sprx", 14)) {
            switch (cachedFirmware) {
                ...
                case 1002:
                    ...
                    _thr_initial = entries[i].start + 0x8A430;
                    break;
                case 1050:
                case 1051:
                case 1070:
        ...
        if (!memcmp(entries[i].name, "libkernel_web.sprx", 18)) {
            switch (cachedFirmware) {
                ...
                case 850:
                    ...
                    _scePthreadCreate = entries[i].start + 0x286A0;
                    _thr_initial = entries[i].start + 0x89430;
                    break;
                case 852:
                    _scePthreadAttrInit = entries[i].start + 0x18D50;
                ...
                case 951:
                case 960:
                    _scePthreadAttrInit = entries[i].start + 0xAA60;
                    _scePthreadAttrSetstacksize = entries[i].start + 0x241E0;
                    _scePthreadCreate = entries[i].start + 0x17160;
                    _thr_initial = entries[i].start + 0x8A430;
                    break;
                case 1000:
                case 1001:
                ...
                case 1150:
                case 1152:
                    _scePthreadAttrInit = entries[i].start + 0x13580;
                    _scePthreadAttrSetstacksize = entries[i].start + 0xD380;
                    _scePthreadCreate = entries[i].start + 0x10230;
                    _thr_initial = entries[i].start + 0x8E430;
                    break;
                case 1200:
                case 1202:
                ...
                case 1350:
                case 1352
                    _scePthreadAttrInit = entries[i].start + 0x8A80;
                    _scePthreadAttrSetstacksize = entries[i].start + 0x38B0;
                    _scePthreadCreate = entries[i].start + 0x16460;
                    _thr_initial = entries[i].start + 0x8E430;
                    break;
            }
            break;
        ...
        if (!memcmp(entries[i].name, "libkernel_sys.sprx", 18)) {
            switch (cachedFirmware) {
                ...
                case 801:
                case 803:
                    _scePthreadAttrInit = entries[i].start + 0x14100;
                    _scePthreadAttrSetstacksize = entries[i].start + 0x14120;
